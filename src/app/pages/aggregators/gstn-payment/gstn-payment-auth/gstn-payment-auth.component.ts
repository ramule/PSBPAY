...
@Component({
  selector: 'app-gstn-payment-auth',
  templateUrl: './gstn-payment-auth.component.html',
  styleUrls: ['./gstn-payment-auth.component.scss']
})
export class GstnPaymentAuthComponent implements OnInit {
  ...
  submitOtpSession(param) {
    ...
    console.log("decrypted req: ", this.tempDecryptedReq)
    ...
    console.log('request: ', this.DataService.request);
    this.submitReq();
  }
    ...
    this.http.callBankingAPIService( this.DataService.request, this.storage.getLocalStorage(this.constant.storage_deviceId),this.DataService.endPoint).subscribe(data => {
      ...
      switch (data.responseParameter.opstatus) {
        ...
        default:
          ...
          this.router.navigate(['/gstnPaymentSuccess']);
      }
   ...
   */
   resendOTP(numCount?: any) {
    this.otpForm.reset();
    var resendOTPReq = this.otpSessionService.getResendOTPSessionReq('GSTNPAYMENT');
    this.http
